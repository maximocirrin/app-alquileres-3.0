...
export function normalizeOwnerContract(contract) {
  ...
  const owner = row(contract.Propietario);
  ...
  return {
    ...
    property: {
      ...
      superficie_cubierta: nullableNumber(property?.superficie_cubierta),
      superficie_lote: nullableNumber(property?.superficie_lote),
      habitaciones_total: nullableNumber(property?.habitaciones_total),
      dormitorios: nullableNumber(property?.dormitorios),
      banos_completos: nullableNumber(property?.banos_completos),
      cantidad_cocheras: nullableNumber(property?.cantidad_cocheras)
    },
    publication: {
      id_publicacion: publicationId,
    ...
    owner: {
      id_perfil: nullableId(owner?.id_perfil) || nullableId(contract.id_perfil_propietario),
      nombre_completo: nullableText(owner?.nombre_completo, 300),
      mail: nullableText(owner?.mail, 320),
      telefono: nullableText(owner?.telefono, 80)
    },
    signatures,
...
/**
 * Returns only contracts where the authenticated profile is one of the two
 * legal participants. Both identities are resolved server-side because Perfil
 * RLS correctly prevents the browser from reading the other person's row.
 */
export async function getParticipantContractsForProfile(supabase, profileId) {
  const participantProfileId = nullableId(profileId);
  if (!participantProfileId) return [];

  const query = supabase
    .from('Contrato')
    .select(OWNER_CONTRACT_SELECT)
    .or(`id_perfil_propietario.eq.${participantProfileId},id_perfil_inquilino.eq.${participantProfileId}`)
    .order('id_contrato', { ascending: false });
  if (typeof query.retry === 'function') query.retry(false);
  const { data, error } = await query;
