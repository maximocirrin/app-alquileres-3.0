...
function guaranteeData(type, value) {
  ...
  for (const [key, maxLength] of fields) {
    const clean = text(source[key], maxLength);
...
function viewModel(invitation, documentCount) {
  return {
    ...
    // The invited person may see their own display name, but no tenant
    // contact data, database ids, document paths, or other guarantors.
    nombre_completo: text(invitation.nombre_completo, 240) || 'Garante',
    relacion_inquilino: text(invitation.relacion_inquilino, 120) || 'Codeudor solidario',
...
/**
 * Public guarantor invitations are bearer capabilities. This route deliberately
 * accepts only high-entropy invitation tokens and keeps all table access and
 * Storage authorizations on the server.
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    const token = invitationToken(body.token);
    ...
    if (action === 'submit') {
      ...
      const { data: updated, error: updateError } = await supabase
        ...
        .update({
          id_estado_garante: 5,
          // Consent evidence is kept with the submitted guarantee payload so
          // this route does not depend on undeclared optional columns.
          datos_garantia: {
            ...data,
      ...
      if (documentsError) {
        // A bulk insert is atomic in PostgREST. Restore the retriable state if
        // it fails after the guarded transition above, so a valid invitation is
        // never left marked as reviewed without its evidence.
        await supabase
          .from('Garante')
