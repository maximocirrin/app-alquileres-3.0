...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (authError || !user) {
      return sendUnauthorized(res, 'Debe iniciar sesión para consultar sus contratos de alquiler.');
    }
    if (!requireProfile(profile)) {
      return sendForbidden(res, 'No se encontró un perfil válido para esta cuenta.');
    }

    const contracts = await getOwnerContractsForProfile(getSupabaseAdmin(), profile.id_perfil);
    return res.status(200).json({ ok: true, data: { contracts } });
  } catch (error) {
    return sendInternalError(res, 'owner-contracts', error);
  }
}
