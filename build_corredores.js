...
const html = fs.readFileSync('propietarios.html', 'utf8');
...
let headMatch = html.match(headRegex);
let head = headMatch ? headMatch[0].replace('<title>Propietarios | Hábitat</title>', '<title>Corredores Inmobiliarios | Hábitat</title>') : '<head></head>';

let navMatch = html.match(navRegex);
let nav = navMatch ? navMatch[0] : '';

let footerMatch = html.match(footerRegex);
let footer = footerMatch ? footerMatch[0] : '';

// get scripts except the ones inside <head> since those are already matched
let scripts = "";
const headEnd = headMatch ? html.indexOf('</head>') : 0;
let match;
...
fs.writeFileSync('corredores.html', finalHtml);
console.log('corredores.html successfully built.');
